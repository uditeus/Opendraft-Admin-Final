...
const AttachmentItem = ({ file, onRemove }: { file: File; onRemove: () => void }) => {
  ...
  return (
    <div className="group relative flex h-20 w-20 flex-col items-center justify-center overflow-hidden rounded-xl border border-border bg-background shadow-sm transition-all hover:shadow-md animate-in fade-in zoom-in-75 duration-200">
      {isImage && preview ? (
        <img src={preview} alt={file.name} className="h-full w-full object-cover" />
      ) : (
        <div className="flex flex-col items-center justify-center p-2 text-center text-muted-foreground">
          <AppIcon name="FileText" className="h-8 w-8 opacity-30" strokeWidth={1.5} />
          <span className="mt-1 max-w-full truncate text-[9px] font-semibold uppercase tracking-wider text-foreground/50">
            {getExt(file.name)}
          </span>
        </div>
      )}
      <button
        onClick={onRemove}
        className="absolute right-0.5 top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-background/80 text-foreground shadow-sm backdrop-blur opacity-0 group-hover:opacity-100 transition-opacity hover:bg-destructive hover:text-destructive-foreground"
      >
            ...
            <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center px-4">
              <div className="pointer-events-auto relative w-full max-w-[760px]">
                ...
                <motion.h1
                  ...
                  className="mb-8 text-center text-[28px] md:text-[32px] font-normal leading-[1.1] tracking-[0.01em] text-foreground"
                >
              ...
              <div className="sticky bottom-0 z-10 w-full bg-background/80 pb-4 pt-2 backdrop-blur-xl">
                ...
                <div className="relative">
                  {attachments.length > 0 && (
                    <div className="chat-container pb-2 flex flex-wrap gap-2">
                      {attachments.map((file, i) => <AttachmentItem key={i} file={file} onRemove={() => setAttachments(p => p.filter((_, idx) => idx !== i))} />)}
                    </div>
                  )}

                  {/* IA Continuity Suggestions */}
                  {(() => {
