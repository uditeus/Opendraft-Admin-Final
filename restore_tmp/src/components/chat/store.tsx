        ...
        sendMessage(
          ...
          try {
            ...
            if (loadError) {
              ...
              const hasAssistant = (dbMessages as any[]).some(m => m.role === 'assistant');
              let finalMessages = dbMessages;

              if (!hasAssistant) {
                console.warn("Post-stream: Assistant message missing from DB. Saving fallback.");
                await appendMessage(actualThreadId, user.id, "assistant", fullText, { thoughtTimeMs: thoughtTimeMsForDb, mode });
              ...
              // DB is totally empty - save both in correct order
              console.warn("Post-stream: DB empty after wait. Saving both messages from frontend.");
              await appendMessage(actualThreadId, user.id, "user", content, { mode });
              await appendMessage(actualThreadId, user.id, "assistant", fullText, { thoughtTimeMs: thoughtTimeMsForDb, mode });
              await loadThreadData(actualThreadId);
            }
          } catch (err) {
